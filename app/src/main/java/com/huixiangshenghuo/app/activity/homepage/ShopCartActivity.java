   ...
   private void initData() {
      ...
      goodAdapter = new CustomBaseAdapter<ShopcartListParam>((ArrayList<ShopcartListParam>) goods, R.layout.item_shop_cart_new) {
         ...
         public void bindView(ViewHolder holder, final ShopcartListParam obj) {
            ...
            if (TextUtils.equals(obj.getType(), CustomConfig.INTEGRAL_GOODS)) {
               priceTxt.setText(CustomConfig.INTEGRAL + obj.getIntegral());
            } else {
               priceTxt.setText(CustomConfig.RMB + obj.getPrice());
            }
...
//        requestObject.setParam(param);
   ...
   public void onClick(View v) {
      switch (v.getId()) {
         ...
         case R.id.asc_buy_txt:
            ...
            if (isContainJFGoods() && isContainNormalGoods()) {
               ToastView.show("不能同时选择积分商品和普通商品");
               return;
            }
   ...
   private boolean isContainJFGoods() {
       for (ShopcartListParam chooseGood : chooseGoods) {
    ...
    private boolean isContainNormalGoods() {
        for (ShopcartListParam chooseGood : chooseGoods) {
           if (chooseGood.getType() != null && TextUtils.equals(chooseGood.getType(), CustomConfig.NORMAL_GOODS)) {
               return true;
           }
        }
        return false;
